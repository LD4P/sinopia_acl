...
Pairs put in the `sinopiaize` configuration will act as a find/replace operation on any matching JSON tree leaves in the 
template data before it is posted to trellis. In this example, anything with a value of `target` will be written as `resource`, etc.: 
```
sinopiaize: {
    'target': 'resource',
    'list': 'resource',
