...
class PopulateStarterResourceTemplates {
  constructor() {
    const gh = new GitHub()
    ...
    this.client = new AuthenticateClient(config.get('cognitoAdminUser'), config.get('cognitoAdminPassword'))
    // for json validation
    this.validateResourceTemplate = new ValidateResourceTemplate()

    this.client.cognitoTokenToFile()
      .then(() => {
        return null
      })
      .catch(error => {
    ...
    this.token = (fs.existsSync(config.get('cognitoTokenFile'))) ?
  ...
  /**
   * Reads in a resource template from the parsed github profile and fixes it to be sinopia-compliant
   * if it was originally a LOC resource template
   * @param resourceTemplate
   */
  sinopiaizeResorceTemplate(resourceTemplate) {
    Object.entries(config.get('sinopiaize')).forEach(([find, replace]) => {
      this.findAndReplace(resourceTemplate, find, replace)
    })
  ...
  /**
   * @param object, which is the resourceTemplate
   * @param value, which is the json object literal value that is searched for
   * @param replacement
   */
  findAndReplace(object, value, replacement) {
    for (const key in object) {
      if (object.hasOwnProperty(key)) {
        if (typeof object[key] === 'object') {
          this.findAndReplace(object[key], value, replacement);
        }
        if (object[key] === value) {
      ...
      this.resourceTemplateShas.forEach(async fileSha => {
        await this.getResourceTemplate(fileSha).then(async blob => {
          ...
          profile.Profile.resourceTemplates.forEach(async resourceTemplate => {
            this.sinopiaizeResorceTemplate(resourceTemplate)
...
const populate = new PopulateStarterResourceTemplates()
